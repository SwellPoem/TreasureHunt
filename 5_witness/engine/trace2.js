...
window.trace = function(event, puzzle, pos, start, symStart=null) {
  if (start != null && data.tracing !== true) { // data.tracing could be undefined or false
    ...
    if (cell.end != null && data.bbox.inMain(data.x, data.y)) {
      ...
      setTimeout(async function() { // Run validation asynchronously so we can free the pointer immediately.
        ...
        if (puzzleData.valid()) {
          ...
          ///////////////////////////////////////////////////// GIULIO
          // - wait 2 seconds and then load the next page
          ...
          setTimeout(function() {
            ...
            var i = parseInt(witness.split("witness")[1]);
            var next = "witness" + (i+1) + ".html";
